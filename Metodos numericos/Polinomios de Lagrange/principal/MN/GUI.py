...
import ttk
import tkMessageBox
from MN.Lagrange import Lagrange
import matplotlib
...
class GUI(tk.Frame):
    ...
    def initComponent(self):
        ...
        self.parent.title("Polinomios de Lagrange")
    ...
    def crearPares(self):
        ...
        if len(grado) != 0 :
            if int(grado) >= 2 and int(grado) <= 10 :
                ...
                lblTemp = tk.Label(self.parent, text="i",
    ...
    def process(self):
        ...
        if self.isEmpty() :
            ...
            # Inicializamos
            lg = Lagrange(self.dato1, self.dato2,
            ...
            areaExpr.place(
                    x=10, y=290, width=780, height=60)
            ...
            lblEc = tk.Label(self.parent, text="Encontrar datos a partir de la ecuación: ",
                  anchor=tk.W, justify=tk.CENTER)
            lblEc.place(x=10, y=360, width=700, height=20)
            lblEc.config(font=self.font, bg=self.bg)
            
            lblTipo = tk.Label(self.parent, text="Ingrese el dato: ",
                  anchor=tk.W, justify=tk.CENTER)
            lblTipo.place(x=10, y=390, width=290, height=20)
            lblTipo.config(font=self.font, bg=self.bg)
            
            self.txtDato = tk.Entry(self.parent)
            self.txtDato.place(x=300, y=390, width=100, height=20)
            self.txtDato.config(font=self.font, bg=self.bg)
            
            btnEc = tk.Button(self.parent, text="Calcular",
                           command=lambda: self.calcular(lg))
            btnEc.place(x=410, y=390, width=180, height=20)
            btnEc.config(font=self.font)
            
            # mostramos graficar
            lblGraph = tk.Label(self.parent, text="Puede graficar la ecuación y los datos pares: ",
                  anchor=tk.W, justify=tk.CENTER)
            lblGraph.place(x=10, y=420, width=700, height=20)
            lblGraph.config(font=self.font, bg=self.bg)
            
            btnGraph = tk.Button(self.parent, text="Graficar",
                           command=lambda: self.graficar(lg))
            btnGraph.place(x=10, y=450, width=200, height=20)
            btnGraph.config(font=self.font)
                
    def calcular(self, lg):
        """
    ...
    def graficar(self, lg):
        ...
        _x = range(-100, 100)
        # Graficar la funcion
        a.plot(_x, [lg.f(lg.ec, i) for i in _x], label="f(x)="+str(lg.ec))
