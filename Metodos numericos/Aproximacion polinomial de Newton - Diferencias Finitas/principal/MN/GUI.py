...
class GUI(tk.Frame):
    ...
    def initComponent(self):
        ...
        self.parent.title("Aproximacion polinomial de Newton - Diferencias finitas")
    ...
    def process(self):
        ...
        if self.isEmpty() :
            ...
            # Inicializamos
            df = DifFin(self.dato1, self.dato2, self.txtPares.get())
            
            # obtenemos la expresion
            expr = df.createExpr()
            
            lblExpr = tk.Label(self.parent, text="Diferencias finitas: ",
                      anchor=tk.W, justify=tk.CENTER)
            lblExpr.place(x=10, y=140, width=700, height=20)
            lblExpr.config(font=self.font, bg=self.bg)
            
            # Mostramos las diferencias finitas
            areaExpr = tk.Text(self.parent)
            areaExpr.place(
                    x=10, y=170, width=780, height=120)
    ...
    def calcular(self, df):
        ...
        if len(dato) > 0:
            ...
            df.initComponent(sp.Float(dato))
            
            lblExpr = tk.Label(self.parent, text="Grado: "+str(df.grado)+" - Dato par de inicio: "+str(df.init),
                      anchor=tk.W, justify=tk.CENTER)
            lblExpr.place(x=10, y=360, width=700, height=20)
            lblExpr.config(font=self.font, bg=self.bg)
            
            # Mostramos las diferencias finitas
            areaExpr = tk.Text(self.parent)
            areaExpr.place(
                    x=10, y=390, width=780, height=60)
            areaExpr.insert(tk.END, df.ecRed)
            areaExpr.config(font=self.font, bg=self.bg)
            areaExpr.config(state="disabled")
