...
class EDO:
    ...
    def euler(self, inicial, final, n):
        ...
        for i in range(0, int(n)) :
            ...
            expr += "x("+str(x[i])+")     y("+str(py[i])+")  (euler)y("+str(y[i])+")\n"
            # almacenamos xi
            y.append(yi)
            
        
        # agregamos los ultimos resultados
        expr += "x("+str(x[int(n)])+")     y("+str(py[int(n)])+")  (euler)y("+str(y[int(n)])+")\n"
    ...
    def eulerModificado(self, inicial, final, n):
        """
        Resolvemos por metodo de euler modificado
        """
        ...
        for i in range(0, int(n)) :
            # obtenemos la derivada (normal)
            diff = self.f(x[i], y[i])
            # Obtenemos xi
            yi = y[i] + (h*diff)
            eci = self.solve(x[i + 1])
            # obtenemos la correccion
            diffCorregida = self.corrector(diff, x[i + 1], yi)
            # Volvemos a obtener yi
            yi = y[i] + (h*diffCorregida)
            # armamos la expresion
    ...
    def corrector(self, diff, x, y):
        """
        Paso corrector para el metodo de Euler modificado
        """
        
        # btenemos la derivada promedio
        res = (diff + self.f(x, y)) / 2.0
