...
class GUI(tk.Frame):
    ...
    def initComponent(self):
        ...
        self.parent.title("Aproximación polinomial por minimos cuadrados")
        ...
        if len(grado) != 0 :
            if int(grado) >= 2 and int(grado) <= 10 :
                ...
                for i in range(0, int(grado)):
                    # Etiqueta de puntos
                    lblTemp = tk.Label(self.parent, text=""+str(i+1),
                              anchor=tk.W, justify=tk.CENTER)
    ...
    def selection_changed(self, event):
        ...
        btnCalcular.place(x=10, y=170, width=150, height=20)
    ...
    def process(self):
        ...
        if self.isEmpty() :
            ...
            # Inicializamos
            mc = MinCua(self.dato1, self.dato2,
                                  self.comboGrado.get())
            ...
            if mc.createMatrix() :
                ...
                lblExpr = tk.Label(self.parent, text="Ecuación formada: ",
                      anchor=tk.W, justify=tk.CENTER)
                lblExpr.place(x=10, y=200, width=700, height=20)
                ...
                for i in range(0, int(grado) + 1):
                    ...
                    lblTemp = tk.Label(self.parent, text="a"+str(i),
                              anchor=tk.W, justify=tk.CENTER)
                    lblTemp.place(x=(10 + (width*i)), y=230,
                                  width=width, height=20)
                    lblTemp.config(font=self.font, bg=self.bg)
                    
                    # datos
                    v = tk.StringVar(self.parent, value=""+str(sp.Float(mc.values[i], 4)))
                    txtTemp = tk.Entry(self.parent, textvariable=v)
                    txtTemp.place(x=(10 + (width*i)), y=250,
                                         width=width, height=20)
                ...
                areaExpr.place(
                        x=10, y=280, width=780, height=60)
    ...
    def graficar(self, inter):
        ...
        _x = range(int(self.dato2[1].get()) - 100, int(self.dato2[len(self.dato2) - 1].get()) + 100)
        ...
        a.set_xlim(int(self.dato2[1].get()) - 100, int(self.dato2[len(self.dato2) - 1].get()) + 100)
        a.set_ylim(int(float(self.dato1[1].get())) - 10, int(float(self.dato1[len(self.dato2) - 1].get())) + 10)
